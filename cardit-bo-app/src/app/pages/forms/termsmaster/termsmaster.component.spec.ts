...
import * as jsonData from './termsmaster.json';

import { termsmasterComponent } from './termsmaster.component';
...
class MockMessageService extends MessageService {
    ...
    add(message: any) {
        ...
        console.log('%c Toast termsmaster ' + message_detail, 'background: #fff; color: red');
    }
    get() {
...
describe('termsmasterComponent', () => {
    ...
    beforeEach(async () => {
        ...
        if (component == undefined) {
            ...
            console.log('%c termsmasterComponent created', strCSS);
        }
    });
    ...
    afterEach(async () => {
        ...
        fixture?.destroy();
        console.log('%c termsmaster fixture deleted', strCSS);
    });

    it('should create', () => {
        expect(component).toBeTruthy();
        console.log('%c termsmasterComponent true', strCSS);
    });
    it('dropdowns functionality', () => {
        console.log('%c termsmaster checking whether dropdowns contain values', strCSSheader);
    ...
    it('Submit & delete functionality', async () => {
        ...
        let json = jsonData;
        console.log('%c termsmaster jsonData ' + (jsonData as any)?.length + '', strCSS);
        console.log('%c  termsmaster JSON Values ' + (json as any)[0] + '', strCSS);
        if (json != undefined && (json as any)?.length > 0 && (json as any)[0].termid) component.termsmaster_Form.controls.termid.setValue((json as any)[0].termid);
        if (json != undefined && (json as any)?.length > 0 && (json as any)[0].termdetails) component.termsmaster_Form.controls.termdetails.setValue((json as any)[0].termdetails);
        if (json != undefined && (json as any)?.length > 0 && (json as any)[0].currentversion) component.termsmaster_Form.controls.currentversion.setValue((json as any)[0].currentversion);
        if (json != undefined && (json as any)?.length > 0 && (json as any)[0].status) component.termsmaster_Form.controls.status.setValue((json as any)[0].status);
        debugger;
        console.log('%c  termsmaster Submitting', strCSSheader);
        ...
        debugger;
        fixture.detectChanges();
        console.log('%c  termsmaster Message: ' + _MockMessageService.get(), strCSS);
        await expect(_MockMessageService.get()).toEqual('Successfully saved');
        await component.PopulateScreen(res.termsmaster.pkcol);

        //expect(window.alert).toHaveBeenCalledWith('Successfully saved');
        console.log('%c  termsmaster Deleting the Record', strCSS);
        ...
        debugger;
        fixture.detectChanges();
        await expect(component.termsmaster_Form.controls.termdetails.value).toEqual(null);
        ...
        console.log('%c  termsmaster -----------------------------------', strCSS);
