...
@Component({
    selector: 'app-termsmaster',
    templateUrl: './termsmaster.component.html',
    styles: [],
...
export class termsmasterComponent implements OnInit {
    ...
    constructor(
        ...
        try {
            ...
            this.termsmaster_Form = this.fb.group({
                pkcol: [null],
                pk: [null],
                termid: [null],
                termdetails: [null, Validators.compose([Validators.required,])],
                currentversion: [null, Validators.compose([Validators.required,])],
                status: [null],
                statusdesc: [null],
            });
        } catch (e) {
    ...
    get f() { return this.termsmaster_Form.controls; }
    ...
    canDeactivate(): Observable<boolean> | boolean {
        //debugger;
        if (this.termsmaster_Form.dirty && this.termsmaster_Form.touched) {
    ...
    async ngOnInit() {
        try {
            ...
            this.viewHtml = ``;
            this.formData = new termsmaster();
            this.PopulateFromMainScreen(this.data, false);
            this.PopulateFromMainScreen(this.dynamicconfig.data, true);
            if (this.currentRoute.snapshot.paramMap.get('tableid') != null) {
                this.ShowTableslist = this.currentRoute.snapshot.paramMap.get('tableid').split(',');
            }
            this.formid = termsmasterid;
            //alert(termsmasterid);
            ...
            this.termsmaster_service.getDefaultData().then(res => {
            }).catch((err) => {
                this.blockedDocument = false;
            ...
            //autocomplete
            this.termsmaster_service.get_termsmasters_List().then(res => {
                this.pkList = res as termsmaster[];
                this.pkoptionsEvent.emit(this.pkList);
            }
            ).catch((err) => {
                this.blockedDocument = false;
                if (this.sharedService.IsDebug) console.log(err);
                this.toastr.addSingle("error", "", 'termsmastersList ' + err);
            });
    ...
    resetForm() {
        this.formid = "";
        this.showview = false;
        if (this.termsmaster_Form != null)
            this.termsmaster_Form.reset();
        this.termsmaster_Form.patchValue({
        });
        this.PopulateFromMainScreen(this.data, false);
        this.PopulateFromMainScreen(this.dynamicconfig.data, true);
    }
    ...
    async onDelete(): Promise<any> {
        let termid = this.termsmaster_Form.get('termid').value;
        if (termid != null) {
            if (confirm('Are you sure to delete this record ?')) {
                let res = await this.termsmaster_service.delete_termsmaster(termid);
    ...
    onCopy() {
        ...
        if (this.formData.termid != null) this.formData.termid = null;
    }
    onCopyDetails() {
        this.formid = null;
        this.termsmaster_Form.patchValue({
    ...
    PopulateFromMainScreen(mainscreendata: any, bdisable: any) {
        if (mainscreendata != null) {
            for (let key in mainscreendata) {
                if (key != 'visiblelist' && key != 'hidelist' && key != 'event' && key != 'termid' && key != 'attachment' && key != 'customfield') {
                    ...
                    if (false)
                        ...
                        this.termsmaster_Form.patchValue({ [key]: mainscreendata[key] });
                    }
                    else {
                        this.termsmaster_Form.patchValue({ [key]: mainscreendata[key] });
                    }
                    {
                        {
                            if (bdisable && this.termsmaster_Form.controls[key] != undefined) {
                                this.termsmaster_Form.controls[key].disable({ onlySelf: true });
    ...
    async PopulateScreen(pkcol: any): Promise<any> {
        try {
            ...
            this.termsmaster_service.get_termsmasters_ByEID(pkcol).then(res => {
                this.blockedDocument = false;

                this.formData = res.termsmaster;
                let formproperty = res.termsmaster.formproperty;
                if (formproperty && formproperty.edit == false) this.showview = true;
                this.pkcol = res.termsmaster.pkcol;
                this.formid = res.termsmaster.termid;
                setTimeout(() => {
                    this.InitializeGrid();
                    this.FillData(res)
                }, 500);
    ...
    FillData(res: any) {
        try {
            ...
            if ((res.termsmaster as any).applicantid == this.sessionService.getItem('applicantid')) this.bmyrecord = true;
            console.log(res);
            //console.log(res.order);
            //console.log(res.orderDetails);
            this.termsmaster_Form.patchValue({
                pkcol: res.termsmaster.pkcol,
                termid: res.termsmaster.termid,
                termdetails: res.termsmaster.termdetails,
                currentversion: res.termsmaster.currentversion,
                status: res.termsmaster.status,
                statusdesc: res.termsmaster.statusdesc,
            });
            this.termsmaster_menuactions = res.termsmaster_menuactions;
            //Child Tables if any
            setTimeout(() => {
            });
        } catch (e) {
    ...
    getHtml(html: any) {
        ...
        for (let key in this.termsmaster_Form.controls) {
            let val = this.termsmaster_Form.controls[key].value;
            if (val == 'null' || val == null || val == undefined) val = '';
            if (this.termsmaster_Form.controls[key] != null) {
                if (false) {
    ...
    GetFormValues() {
        let formData: any;
        formData = this.termsmaster_Form.getRawValue();
        ...
        formData.termid = this.formid;
        return formData;
    }
    async onSubmitDataDlg(bclear: any) {
        this.isSubmitted = true;
        if (!this.termsmaster_Form.valid) {
    ...
    async onSubmitData(bclear: any): Promise<any> {
        try {
            ...
            this.isSubmitted = true;
            let strError = "";
            Object.keys(this.termsmaster_Form.controls).forEach(key => {
                const controlErrors: ValidationErrors = this.termsmaster_Form.get(key).errors;
