import { customerrecipientlinkService } from './../../../service/customerrecipientlink.service';
...
@Component({
    selector: 'app-customerrecipientlink',
    templateUrl: './customerrecipientlink.component.html',
    styles: [],
...
export class customerrecipientlinkComponent implements OnInit {
    blockedDocument: boolean = false;
    formData: customerrecipientlink;
    list: customerrecipientlink[];
    ...
    customerrecipientlink_Form: FormGroup;
    ...
    constructor(
        ...
        try {
            ...
            this.customerrecipientlink_Form = this.fb.group({
                pkcol: [null],
                pk: [null],
                linkid: [null],
                customerid: [null],
                customeriddesc: [null],
                uid: [null, Validators.compose([Validators.maxLength(32)])],
                uiddesc: [null],
                recipientuid: [null],
    ...
    get f() { return this.customerrecipientlink_Form.controls; }
    ...
    canDeactivate(): Observable<boolean> | boolean {
        //debugger;
        if (this.customerrecipientlink_Form.dirty && this.customerrecipientlink_Form.touched) {
    ...
    async ngOnInit() {
        try {
            ...
            let customerrecipientlinkid = null;
            ...
            this.viewHtml = ``;
            this.formData = new customerrecipientlink();
            this.PopulateFromMainScreen(this.data, false);
            this.PopulateFromMainScreen(this.dynamicconfig.data, true);
            if (this.currentRoute.snapshot.paramMap.get('tableid') != null) {
                this.ShowTableslist = this.currentRoute.snapshot.paramMap.get('tableid').split(',');
            }
            this.formid = customerrecipientlinkid;
            //alert(customerrecipientlinkid);
            ...
            this.customerrecipientlink_service.getDefaultData().then(res => {
                ...
                if (this.formData?.customerid != undefined && this.formData?.customerid != null) {
                    this.customerrecipientlink_Form.patchValue({
            ...
            //autocomplete
            this.customerrecipientlink_service.get_customerrecipientlinks_List().then(res => {
                this.pkList = res as customerrecipientlink[];
                this.pkoptionsEvent.emit(this.pkList);
            }
            ).catch((err) => {
                this.blockedDocument = false;
                if (this.sharedService.IsDebug) console.log(err);
                this.toastr.addSingle("error", "", 'customerrecipientlinksList ' + err);
            });
    ...
    resetForm() {
        this.formid = "";
        this.showview = false;
        if (this.customerrecipientlink_Form != null)
    ...
    async onDelete(): Promise<any> {
        let linkid = this.customerrecipientlink_Form.get('linkid').value;
        if (linkid != null) {
            if (confirm('Are you sure to delete this record ?')) {
                let res = await this.customerrecipientlink_service.delete_customerrecipientlink(linkid);
    ...
    onCopy() {
        ...
        if (this.formData.linkid != null) this.formData.linkid = null;
    }
    onCopyDetails() {
        this.formid = null;
        this.customerrecipientlink_Form.patchValue({
    ...
    PopulateFromMainScreen(mainscreendata: any, bdisable: any) {
        if (mainscreendata != null) {
            for (let key in mainscreendata) {
                if (key != 'visiblelist' && key != 'hidelist' && key != 'event' && key != 'linkid' && key != 'attachment' && key != 'customfield') {
                    ...
                    if (false)
                        ...
                        this.customerrecipientlink_Form.patchValue({ [key]: mainscreendata[key] });
                    }
                    else {
                        this.customerrecipientlink_Form.patchValue({ [key]: mainscreendata[key] });
                    }
                    {
                        {
                            if (bdisable && this.customerrecipientlink_Form.controls[key] != undefined) {
                                this.customerrecipientlink_Form.controls[key].disable({ onlySelf: true });
    ...
    async PopulateScreen(pkcol: any): Promise<any> {
        try {
            ...
            this.customerrecipientlink_service.get_customerrecipientlinks_ByEID(pkcol).then(res => {
                this.blockedDocument = false;

                this.formData = res.customerrecipientlink;
                let formproperty = res.customerrecipientlink.formproperty;
                if (formproperty && formproperty.edit == false) this.showview = true;
                this.pkcol = res.customerrecipientlink.pkcol;
                this.formid = res.customerrecipientlink.linkid;
                setTimeout(() => {
                    this.InitializeGrid();
                    this.FillData(res)
                }, 500);
    ...
    FillData(res: any) {
        try {
            ...
            if ((res.customerrecipientlink as any).applicantid == this.sessionService.getItem('applicantid')) this.bmyrecord = true;
            console.log(res);
            //console.log(res.order);
            //console.log(res.orderDetails);
            this.customerrecipientlink_Form.patchValue({
                pkcol: res.customerrecipientlink.pkcol,
                linkid: res.customerrecipientlink.linkid,
                customerid: this.sharedService.getValue('', this.customerid_List, res.customerrecipientlink.customerid, 'customerid'),
                customeriddesc: res.customerrecipientlink.customeriddesc,
                uid: this.sharedService.getValue('uid', this.uid_List, res.customerrecipientlink.uid, 'uid'),
                uiddesc: res.customerrecipientlink.uiddesc,
                recipientuid: this.sharedService.getValue('uid', this.recipientuid_List, res.customerrecipientlink.recipientuid, 'recipientuid'),
                recipientuiddesc: res.customerrecipientlink.recipientuiddesc,
                status: res.customerrecipientlink.status,
                statusdesc: res.customerrecipientlink.statusdesc,
            });
            this.customerrecipientlink_menuactions = res.customerrecipientlink_menuactions;
            //Child Tables if any
            setTimeout(() => {
            });
        } catch (e) {
    ...
    getHtml(html: any) {
        ...
        for (let key in this.customerrecipientlink_Form.controls) {
            let val = this.customerrecipientlink_Form.controls[key].value;
            if (val == 'null' || val == null || val == undefined) val = '';
            if (this.customerrecipientlink_Form.controls[key] != null) {
                if (false) {
    ...
    GetFormValues() {
        let formData: any;
        formData = this.customerrecipientlink_Form.getRawValue();
        formData.customerid = (this.customerrecipientlink_Form.get('customerid'))?.value?.value;
        formData.uid = (this.customerrecipientlink_Form.get('uid'))?.value?.uid;
        formData.recipientuid = (this.customerrecipientlink_Form.get('recipientuid'))?.value?.uid;
        ...
        formData.linkid = this.formid;
        return formData;
    }
    async onSubmitDataDlg(bclear: any) {
        this.isSubmitted = true;
        if (!this.customerrecipientlink_Form.valid) {
    ...
    async onSubmitData(bclear: any): Promise<any> {
        try {
            ...
            this.isSubmitted = true;
            let strError = "";
            Object.keys(this.customerrecipientlink_Form.controls).forEach(key => {
                const controlErrors: ValidationErrors = this.customerrecipientlink_Form.get(key).errors;
            ...
            this.blockedDocument = true;
            let res = await this.customerrecipientlink_service.save_customerrecipientlinks(this.formData);
            this.blockedDocument = false;
            //debugger;
            this.toastr.addSingle("success", "", "Successfully saved");
            this.objvalues.push((res as any).customerrecipientlink);
