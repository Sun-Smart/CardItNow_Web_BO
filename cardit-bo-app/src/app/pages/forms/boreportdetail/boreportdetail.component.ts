...
export class boreportdetailComponent implements OnInit {
    ...
    separator_Suggestions: any[];
    separator_optionsEvent: EventEmitter<any> = new EventEmitter<any>();//autocomplete
    ...
    constructor(
        ...
        try {
            ...
            this.boreportdetail_Form = this.fb.group({
                pkcol: [null],
                pk: [null],
                reportdetailid: [null, Validators.compose([Validators.pattern('^[0-9]+([0-9]{0,})?$'), Validators.max(100000000),])],
                reportid: [null, Validators.compose([Validators.pattern('^[0-9]+([0-9]{0,})?$'), Validators.max(100000000),])],
                tablename: [null, Validators.compose([Validators.required, Validators.maxLength(100)])],
                formula: [null],
                separator: [null, Validators.compose([Validators.maxLength(10)])],
                separatordesc: [null],
                header: [null],
                footer: [null],
                wherecondition: [null, Validators.compose([Validators.required,])],
                alias: [null, Validators.compose([Validators.required, Validators.maxLength(10)])],
                status: [null],
                statusdesc: [null],
            });
        } catch (e) {
    ...
    async ngOnInit() {
        try {
            ...
            this.viewHtml = ``;
            this.formData = new boreportdetail();
            ...
            this.boreportdetail_service.getDefaultData().then(res => {
                ...
                if (this.formData?.separator != undefined && this.formData?.separator != null) {
                    this.boreportdetail_Form.patchValue({
                        separator: this.formData.separator
                    });
                }
            }).catch((err) => {
                this.blockedDocument = false;
            ...
            this.boreportdetail_service.get_boreportdetails_List().then(res => {
                ...
                this.blockedDocument = false;
                if (this.sharedService.IsDebug) console.log(err);
                this.toastr.addSingle("error", "", 'boreportdetailsList ' + err);
            });
    ...
    onEntered_separator(value: any) {
        this.separator_Suggestions = this.separator_List?.filter(v => v["label"] != null && v["label"]?.toString().toLowerCase().indexOf(value.query.toLowerCase()) > -1);
    }
    onSelected_separator(separatorDetail: any) {
        if (separatorDetail.value && separatorDetail) {
    ...
    async onDelete(): Promise<any> {
        ...
        if (reportdetailid != null) {
            if (confirm('Are you sure to delete this record ?')) {
                let res = await this.boreportdetail_service.delete_boreportdetail(reportdetailid);
    ...
    PopulateFromMainScreen(mainscreendata: any, bdisable: any) {
        if (mainscreendata != null) {
            for (let key in mainscreendata) {
                if (key != 'visiblelist' && key != 'hidelist' && key != 'event' && key != 'reportdetailid' && key != 'attachment' && key != 'customfield') {
    ...
    FillData(res: any) {
        try {
            ...
            this.boreportdetail_Form.patchValue({
                pkcol: res.boreportdetail.pkcol,
                ...
                separator: this.sharedService.getValue("value", this.separator_List, res.boreportdetail.separator, 'separator'),
    ...
    GetFormValues() {
        let formData: any;
        formData = this.boreportdetail_Form.getRawValue();
        formData.separator = (this.boreportdetail_Form.get('separator'))?.value?.value;
        ...
        formData.reportdetailid = this.formid;
        return formData;
    }
    ...
    async onSubmitData(bclear: any): Promise<any> {
        try {
            ...
            this.blockedDocument = true;
            let res = await this.boreportdetail_service.save_boreportdetails(this.formData);
            this.blockedDocument = false;
            //debugger;
