...
export class citymasterComponent implements OnInit {
    ...
    constructor(
        ...
        try {
            ...
            this.citymaster_Form = this.fb.group({
                pkcol: [null],
                pk: [null],
                cityid: [null],
                geoid: [null, Validators.compose([Validators.required,])],
                geoiddesc: [null],
                cityname: [null, Validators.compose([Validators.required, Validators.maxLength(100)])],
                status: [null],
                statusdesc: [null],
            });
        } catch (e) {
    ...
    get f() { return this.citymaster_Form.controls; }
    ...
    canDeactivate(): Observable<boolean> | boolean {
        //debugger;
        if (this.citymaster_Form.dirty && this.citymaster_Form.touched) {
    ...
    //check Unique fields
    citynameexists(e: any) {
        try {
            //debugger;
            let pos = this.pkList.map(function (e: any) { return e.cityname?.toString().toLowerCase(); }).indexOf(e.target.value?.toString().toLowerCase());

            if (pos >= 0 && this.pkList[pos].cityid?.toString() != this.formid?.toString()) {
                if (confirm("This City Name value exists in the database.Do you want to display the record ? ")) {
    ...
    async ngOnInit() {
        try {
            ...
            this.viewHtml = ``;
            this.formData = new citymaster();
            this.PopulateFromMainScreen(this.data, false);
            this.PopulateFromMainScreen(this.dynamicconfig.data, true);
            if (this.currentRoute.snapshot.paramMap.get('tableid') != null) {
                this.ShowTableslist = this.currentRoute.snapshot.paramMap.get('tableid').split(',');
            }
            this.formid = citymasterid;
            //alert(citymasterid);
            ...
            this.citymaster_service.getDefaultData().then(res => {
                ...
                if (this.formData?.geoid != undefined && this.formData?.geoid != null) {
                    this.citymaster_Form.patchValue({
            ...
            //autocomplete
            this.citymaster_service.get_citymasters_List().then(res => {
                this.pkList = res as citymaster[];
                this.pkoptionsEvent.emit(this.pkList);
            }
            ).catch((err) => {
                this.blockedDocument = false;
                if (this.sharedService.IsDebug) console.log(err);
                this.toastr.addSingle("error", "", 'citymastersList ' + err);
            });
    ...
    async onDelete(): Promise<any> {
        let cityid = this.citymaster_Form.get('cityid').value;
        if (cityid != null) {
            if (confirm('Are you sure to delete this record ?')) {
                let res = await this.citymaster_service.delete_citymaster(cityid);
    ...
    PopulateFromMainScreen(mainscreendata: any, bdisable: any) {
        if (mainscreendata != null) {
            for (let key in mainscreendata) {
                if (key != 'visiblelist' && key != 'hidelist' && key != 'event' && key != 'cityid' && key != 'attachment' && key != 'customfield') {
                    ...
                    if (false)
                        ...
                        this.citymaster_Form.patchValue({ [key]: mainscreendata[key] });
                    }
                    else {
                        this.citymaster_Form.patchValue({ [key]: mainscreendata[key] });
                    }
                    {
                        {
                            if (bdisable && this.citymaster_Form.controls[key] != undefined) {
                                this.citymaster_Form.controls[key].disable({ onlySelf: true });
    ...
    async PopulateScreen(pkcol: any): Promise<any> {
        try {
            ...
            this.citymaster_service.get_citymasters_ByEID(pkcol).then(res => {
                this.blockedDocument = false;

                this.formData = res.citymaster;
                let formproperty = res.citymaster.formproperty;
                if (formproperty && formproperty.edit == false) this.showview = true;
                this.pkcol = res.citymaster.pkcol;
                this.formid = res.citymaster.cityid;
                setTimeout(() => {
                    this.InitializeGrid();
                    this.FillData(res)
                }, 500);
    ...
    FillData(res: any) {
        try {
            ...
            if ((res.citymaster as any).applicantid == this.sessionService.getItem('applicantid')) this.bmyrecord = true;
            console.log(res);
            //console.log(res.order);
            //console.log(res.orderDetails);
            this.citymaster_Form.patchValue({
                pkcol: res.citymaster.pkcol,
                cityid: res.citymaster.cityid,
                geoid: this.sharedService.getValue('', this.geoid_List, res.citymaster.geoid, 'geoid'),
                geoiddesc: res.citymaster.geoiddesc,
                cityname: res.citymaster.cityname,
                status: res.citymaster.status,
                statusdesc: res.citymaster.statusdesc,
            });
    ...
    getHtml(html: any) {
        ...
        for (let key in this.citymaster_Form.controls) {
            let val = this.citymaster_Form.controls[key].value;
            if (val == 'null' || val == null || val == undefined) val = '';
            if (this.citymaster_Form.controls[key] != null) {
                if (false) {
    ...
    GetFormValues() {
        let formData: any;
        formData = this.citymaster_Form.getRawValue();
        formData.geoid = (this.citymaster_Form.get('geoid'))?.value?.value;
        ...
        formData.cityid = this.formid;
        return formData;
    }
    async onSubmitDataDlg(bclear: any) {
        this.isSubmitted = true;
        if (!this.citymaster_Form.valid) {
    ...
    async onSubmitData(bclear: any): Promise<any> {
        try {
            ...
            this.isSubmitted = true;
            let strError = "";
            Object.keys(this.citymaster_Form.controls).forEach(key => {
                const controlErrors: ValidationErrors = this.citymaster_Form.get(key).errors;
