...
import * as jsonData from './menuaccess.json';
...
class MockMessageService extends MessageService {
    ...
    add(message: any) {
        ...
        console.log('%c Toast menuaccess ' + message_detail, 'background: #fff; color: red');
    }
    get() {
...
describe('menuaccessComponent', () => {
    ...
    beforeEach(async () => {
        ...
        if (component == undefined) {
            ...
            console.log('%c menuaccessComponent created', strCSS);
        }
    });
    ...
    afterEach(async () => {
        ...
        fixture?.destroy();
        console.log('%c menuaccess fixture deleted', strCSS);
    });

    it('should create', () => {
        expect(component).toBeTruthy();
        console.log('%c menuaccessComponent true', strCSS);
    });
    it('dropdowns functionality', () => {
        console.log('%c menuaccess checking whether dropdowns contain values', strCSSheader);

        fixture.whenStable();
        console.log('%c menuaccess menuid_List ' + component.menuid_List?.length + '', strCSS);
        console.log('%c menuaccess roleid_List ' + component.roleid_List?.length + '', strCSS);
    ...
    it('Submit & delete functionality', async () => {
        ...
        let json = jsonData;
        console.log('%c menuaccess jsonData ' + (jsonData as any)?.length + '', strCSS);
        console.log('%c  menuaccess JSON Values ' + (json as any)[0] + '', strCSS);
        if (json != undefined && (json as any)?.length > 0 && (json as any)[0].menuaccessid) component.menuaccess_Form.controls.menuaccessid.setValue((json as any)[0].menuaccessid);
        if (json != undefined && (json as any)?.length > 0 && (json as any)[0].menuid) component.menuaccess_Form.controls.menuid.setValue((json as any)[0].menuid);
        if (json != undefined && (json as any)?.length > 0 && (json as any)[0].roleid) component.menuaccess_Form.controls.roleid.setValue((json as any)[0].roleid);
        if (json != undefined && (json as any)?.length > 0 && (json as any)[0].status) component.menuaccess_Form.controls.status.setValue((json as any)[0].status);
        debugger;
        console.log('%c  menuaccess Submitting', strCSSheader);
        ...
        debugger;
        fixture.detectChanges();
        console.log('%c  menuaccess Message: ' + _MockMessageService.get(), strCSS);
        await expect(_MockMessageService.get()).toEqual('Successfully saved');
        await component.PopulateScreen(res.menuaccess.pkcol);

        //expect(window.alert).toHaveBeenCalledWith('Successfully saved');
        console.log('%c  menuaccess Deleting the Record', strCSS);
        ...
        debugger;
        fixture.detectChanges();
        await expect(component.menuaccess_Form.controls.menuid.value).toEqual(null);
        ...
        console.log('%c  menuaccess -----------------------------------', strCSS);
