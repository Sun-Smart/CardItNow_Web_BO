...
export class menuaccessComponent implements OnInit {
    ...
    constructor(
        ...
        try {
            ...
            this.menuaccess_Form = this.fb.group({
                pkcol: [null],
                pk: [null],
                menuaccessid: [null],
    ...
    get f() { return this.menuaccess_Form.controls; }
    ...
    canDeactivate(): Observable<boolean> | boolean {
        //debugger;
        if (this.menuaccess_Form.dirty && this.menuaccess_Form.touched) {
    ...
    prev() {
        //debugger;
        let pos = this.pkList.map(function (e: any) { return e.menuaccessid?.toString(); }).indexOf(this.formid?.toString());
    ...
    onSelectedpk(pkDetail: any) {
        if (pkDetail.menuaccessid && pkDetail) {
    ...
    async ngOnInit() {
        try {
            ...
            this.viewHtml = ``;
            this.formData = new menuaccess();
            this.PopulateFromMainScreen(this.data, false);
            this.PopulateFromMainScreen(this.dynamicconfig.data, true);
            if (this.currentRoute.snapshot.paramMap.get('tableid') != null) {
                this.ShowTableslist = this.currentRoute.snapshot.paramMap.get('tableid').split(',');
            }
            this.formid = menuaccessid;
            //alert(menuaccessid);
            ...
            this.menuaccess_service.getDefaultData().then(res => {
                ...
                if (this.formData?.menuid != undefined && this.formData?.menuid != null) {
                    this.menuaccess_Form.patchValue({
                        menuid: this.sharedService.getValue('', this.menuid_List, this.formData.menuid?.toString(), 'menuid')
                    });
                }
            ...
            //autocomplete
            this.menuaccess_service.get_menuaccesses_List().then(res => {
                this.pkList = res as menuaccess[];
                this.pkoptionsEvent.emit(this.pkList);
            }
            ).catch((err) => {
                this.blockedDocument = false;
                if (this.sharedService.IsDebug) console.log(err);
                this.toastr.addSingle("error", "", 'menuaccessesList ' + err);
            });
    ...
    async onDelete(): Promise<any> {
        let menuaccessid = this.menuaccess_Form.get('menuaccessid').value;
        if (menuaccessid != null) {
            if (confirm('Are you sure to delete this record ?')) {
                let res = await this.menuaccess_service.delete_menuaccess(menuaccessid);
    ...
    onCopy() {
        ...
        if (this.formData.menuaccessid != null) this.formData.menuaccessid = null;
    }
    onCopyDetails() {
        this.formid = null;
        this.menuaccess_Form.patchValue({
    ...
    PopulateFromMainScreen(mainscreendata: any, bdisable: any) {
        if (mainscreendata != null) {
            for (let key in mainscreendata) {
                if (key != 'visiblelist' && key != 'hidelist' && key != 'event' && key != 'menuaccessid' && key != 'attachment' && key != 'customfield') {
                    ...
                    if (false)
                        ...
                        this.menuaccess_Form.patchValue({ [key]: mainscreendata[key] });
                    }
                    else {
                        this.menuaccess_Form.patchValue({ [key]: mainscreendata[key] });
                    }
                    {
                        {
                            if (bdisable && this.menuaccess_Form.controls[key] != undefined) {
                                this.menuaccess_Form.controls[key].disable({ onlySelf: true });
    ...
    async PopulateScreen(pkcol: any): Promise<any> {
        try {
            ...
            this.menuaccess_service.get_menuaccesses_ByEID(pkcol).then(res => {
                this.blockedDocument = false;

                this.formData = res.menuaccess;
                let formproperty = res.menuaccess.formproperty;
                if (formproperty && formproperty.edit == false) this.showview = true;
                this.pkcol = res.menuaccess.pkcol;
                this.formid = res.menuaccess.menuaccessid;
                setTimeout(() => {
                    this.InitializeGrid();
                    this.FillData(res)
                }, 500);
    ...
    FillData(res: any) {
        try {
            ...
            if ((res.menuaccess as any).applicantid == this.sessionService.getItem('applicantid')) this.bmyrecord = true;
            console.log(res);
            //console.log(res.order);
            //console.log(res.orderDetails);
            this.menuaccess_Form.patchValue({
                pkcol: res.menuaccess.pkcol,
                menuaccessid: res.menuaccess.menuaccessid,
                menuid: this.sharedService.getValue('', this.menuid_List, res.menuaccess.menuid, 'menuid'),
                menuiddesc: res.menuaccess.menuiddesc,
                roleid: this.sharedService.getValue('', this.roleid_List, res.menuaccess.roleid, 'roleid'),
                roleiddesc: res.menuaccess.roleiddesc,
                status: res.menuaccess.status,
                statusdesc: res.menuaccess.statusdesc,
            });
    ...
    getHtml(html: any) {
        ...
        for (let key in this.menuaccess_Form.controls) {
            let val = this.menuaccess_Form.controls[key].value;
            if (val == 'null' || val == null || val == undefined) val = '';
            if (this.menuaccess_Form.controls[key] != null) {
                if (false) {
    ...
    GetFormValues() {
        ...
        formData.menuaccessid = this.formid;
        return formData;
    }
    async onSubmitDataDlg(bclear: any) {
        this.isSubmitted = true;
        if (!this.menuaccess_Form.valid) {
    ...
    async onSubmitData(bclear: any): Promise<any> {
        ...
        try {
            ...
            this.isSubmitted = true;
            let strError = "";
            Object.keys(this.menuaccess_Form.controls).forEach(key => {
                const controlErrors: ValidationErrors = this.menuaccess_Form.get(key).errors;
            ...
            this.blockedDocument = true;
            let res = await this.menuaccess_service.save_menuaccesses(this.formData);
            this.blockedDocument = false;
            //debugger;
            this.toastr.addSingle("success", "", "Successfully saved");
            this.objvalues.push((res as any).menuaccess);
